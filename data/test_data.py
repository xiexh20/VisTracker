...
class TestData(BaseDataset):
    def __init__(self, data_paths, batch_size, num_workers,
        ...
        if camera_params is not None:
            ...
            self.camera = KinectColorCamera(**camera_params)
        else:
            camera_params = {}
            self.camera = KinectColorCamera(crop_size)
        print("Test dataloader using camera parameters:", camera_params)
    ...
    def prepare_image_crop(self, rgb_file, flip):
        ...
        #                                                                                                       width[1])
        assert bmax[0] > 0, f'no valid mask found for image {rgb_file}'
