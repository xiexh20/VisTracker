...
class ReconFitterBase:
    def __init__(self, seq_folder,
        ...
        if 'camera_params' not in args:
            ...
            self.camera = KinectColorCamera(**args.camera_params)
        ...
        self.debug = debug
        self.vis_idx = 2 # example index in a batch for debug visualization
        self.collision_loss = 'gpu20' in socket.gethostname() or 'gpu16' in socket.gethostname()
        if not self.collision_loss:
            print("Warning: not computing collision loss, should be used for debug only!")
        self.part_names={
            0:'head',
            1:'left foot',
            2:'left hand',
    ...
    def compute_pca_init(self, obj_name):
        ...
        scan = load_template(obj_name) # load template from BEHAVE objects or InterCap objects
        self.scan = scan
        obj_verts = scan.v
    ...
    @staticmethod
    def init_object_orientation(tgt_axis, src_axis, mute=False):
        """
        ...
        if not mute:
            print("Source PCA determinat:", torch.det(src_axis), 'target PCA determinat:', torch.det(tgt_axis)) # boxlong t3.0: -0.06
    ...
    def is_done(self, image_paths, save_name, test_id, neural_only=False):
        if neural_only:
            for i, x in enumerate(image_paths):
                ...
                npz_file = join(folder, f'k{test_id}_densepc.npz')
            ...
            for sf, of in zip(smpl_files, obj_files):
                sf = sf.replace('.ply', '.pkl')
                of = of.replace('.ply', '.pkl') # Nov 3: only check parameters
    ...
    def save_outputs(self, smpl, obj_R, obj_t, traindata_paths, save_name, test_id, obj_s=None):
        ...
        opt_utils.save_smplfits(smpl_files, np.zeros(len(obj_t)), smpl, save_mesh=False) # Nov. 3: not saving meshes
        ...
        for v, p, r, s, t in zip(verts, obj_files, obj_R.detach().cpu().numpy(), obj_s.detach().cpu().numpy(), obj_t.detach().cpu().numpy()):
            ...
            # m.write_ply(p) # Nov. 3: not saving meshes
    ...
    def load_occ_ratios(self, rgb_files, use_pred=False, recon_name=None):
        "load object occlusion ratios, return: (B,) in self.device"
        if use_pred:
            return self.load_occ_ratios_recon(rgb_files, recon_name) # load from recon predicted visibility
        seq_name = DataPaths.get_seq_name(rgb_files[0])
        gtpack_file = osp.join(GT_PACKED, f'{seq_name}_GT-packed.pkl')
        data = joblib.load(gtpack_file)
        frame_inds, kids = self.extract_frame_inds(data, rgb_files)

        occ_ratios = data['occ_ratios']
        occ_ret = np.array([occ_ratios[fid, kid] for fid, kid in zip(frame_inds, kids)])
        print(frame_inds[:3], rgb_files[:3])
        return torch.from_numpy(occ_ret).float().to(self.device)

    def extract_frame_inds(self, data, rgb_files):
        "extract frame inds for the given rgb files"
        kids = [DataPaths.get_kinect_id(file) for file in rgb_files]
        frames = [DataPaths.rgb2seq_frame(file)[1] for file in rgb_files]
        frame_times = data['frames']
        frame_inds = [frame_times.index(x) for x in frames]
        return frame_inds, kids
    ...
    def load_occ_ratios_recon(self, rgb_files,
        ...
        for i in range(len(kids)):
            assert kids[i] == test_kid, f'{frame_times[i]} kinect id={kids[i]}!= test kid={test_kid}'

        occ_ret = np.array([occ_ratios[fid] for fid, kid in zip(frame_inds, kids)])
        print(frame_inds[:3], rgb_files[:3])
        return torch.from_numpy(occ_ret).float().to(self.device)
    ...
    def get_test_files(self, args):
        image_files = DataPaths.get_image_paths_seq(self.seq_folder, args.test_kid, check_occlusion=False, pat=args.pat)
        # print(image_files)
        batch_end = args.end if args.end is not None else len(image_files)
        if args.pat != 't*' and image_files[0].startswith("/BS/xxie-4/static00/behave-fps30") and args.end is not None:
            batch_end = batch_end // 30
            args.start = args.start // 30
    ...
    def prep_smplfit(self, data, generator, pc_generated):
        ...
        betas_dict, body_kpts, human_parts, human_points, obj_points, part_colors, part_labels, query_dict = self.prep_smplfit_dict(
            data, generator, pc_generated, smpl)
        return betas_dict, body_kpts, human_parts, human_points, human_t, obj_points, part_colors, part_labels, query_dict, smpl

    def prep_smplfit_dict(self, data, generator, pc_generated, smpl):
        "SMPL fit data dictionary"
        batch_size = data['images'].shape[0]
        human_points = pc_generated['human']['points'].clone().detach().cpu()
        obj_points = pc_generated['object']['points'].clone().detach().cpu()
        human_parts = pc_generated['human']['parts'].clone().detach().cpu()
        ...
        return betas_dict, body_kpts, human_parts, human_points, obj_points, part_colors, part_labels, query_dict

    def get_smpl_translation(self, data, pc_generated):
        """
        compute SMPL init translation, return (B, 3)
        Args:
            data: a batch of data from dataloader
            pc_generated:

        Returns: SMPL center initialization (B, 3)
    ...
    def visualize_fitting(self, data_dict, object, smpl, smpl_verts):
        ...
        cv2.waitKey(10)
        obj = Mesh(object[idx].detach().cpu().numpy(), [], vc='red') # current object
    ...
    def compute_df_h_loss(self, data_dict, loss_dict, model, smpl_verts):
        ...
        df_pred, pca_pred, parts_pred, centers_pred = model.get_preds()[:4]
    ...
    def update_query_dict(self, points, query_dict):
        """
        recompute the query input dictionary, will be overload by warping fileds
        Args:
