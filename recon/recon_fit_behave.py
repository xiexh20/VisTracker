...
class ReconFitterBehave(ReconFitterBase):
    def fit_recon(self, args):
        ...
        for i, data in enumerate(loop):
            ...
            # self.save_neural_recon(data['path'], pc_generated, args.save_name, args.test_kid)
            # continue
            ...
            try:
                ...
                print("Point generation error:", e)
                continue
            ...
            # obtain object init
            batch_size = data['images'].shape[0]
            scale = torch.ones(batch_size).to(self.device)  # Oct21: use single scale
            ...
            batch_end = time.time()
            print(f"Optimization time for one batch of size {batch_size}: {batch_end - batch_start:.4f} seconds")

    def get_camera_params(self, args):
        if 'camera_params' not in args:
            camera_params = {}
            print("Using default BEHAVE cameras")
        else:
            camera_params = args.camera_params
            print("Using camera parameters:", args.camera_params)
        return camera_params

    def generate_all(self, args, data, generator, num_points=4000):
        "generate pc for a full batch by separating it into small batches"
        hostname = socket.gethostname()
        mini_batch_size = 8 if 'volta' in hostname else 16

        pc_generated_all = []
        samples_count = 100000  # number of samples
        batch_size = data['images'].shape[0]
        # if not 'volta' in hostname:
        #     assert batch_size > 2, f'should not use batch size <=2 in {hostname} machine!'
        for start_idx in range(0, batch_size, mini_batch_size):
            mini_batch = {}
            for k in data:
                mini_batch[k] = data[k][start_idx:start_idx + mini_batch_size]
            # split to small batches
            start_time = time.time()
            pc_generated = generator.generate_pclouds_batch(mini_batch, num_points=num_points, num_steps=10, mute=True)
            self.save_neural_recon(mini_batch['path'], pc_generated, args.save_name, args.test_kid)
            samples_count = np.min([pc_generated['human']['points'].shape[1],
                                    pc_generated['object']['points'].shape[1],
                                    samples_count])
            pc_generated_all.append(pc_generated)
            end_time = time.time()
            total_time = end_time - start_time
            print(f'minibatch {start_idx / mini_batch_size} done, total time {total_time} seconds, avg time: {total_time/mini_batch_size}')
        # then combine
        pc_generated_comb = self.combine_mini_batches(pc_generated_all, samples_count)
        pc_generated = pc_generated_comb
        torch.cuda.empty_cache()
        return pc_generated

    def combine_mini_batches(self, pc_generated_all, samples_count):
        "combine generated results of several mini batches"
        pc_generated_comb = {
            'human': {
                'points': torch.zeros(0, samples_count, 3),
                'parts': torch.zeros(0, samples_count),
                'centers': torch.zeros(0, 6),
                'pca_axis': torch.zeros(0, 3, 3),
                'visibility': torch.zeros(0,),
            },
            'object': {
                'points': torch.zeros(0, samples_count, 3),
                'parts': torch.zeros(0, samples_count),
        ...
        for pc_gen in pc_generated_all:
            for target in pc_generated_comb.keys():
                # for pred_type in pc_generated_comb[target].keys():
                for pred_type in pc_gen[target].keys():
                    if pred_type in ['points', 'parts']:
                        # pick N samples
                        pc_generated_comb[target][pred_type] = torch.cat([pc_generated_comb[target][pred_type],
                                                                          pc_gen[target][pred_type][:, :samples_count]],
                                                                         0)
                    else:
                        # direct concat
                        pc_generated_comb[target][pred_type] = torch.cat([pc_generated_comb[target][pred_type],
                                                                          pc_gen[target][pred_type]], 0)
        return pc_generated_comb
    ...
    def init_dataloader(self, args):
        ...
        camera_params = None if 'camera_params' not in args else args.camera_params
        dataset = TestData(image_files, batch_size, min(batch_size, 10),
    ...
    def optimize_smpl_object(self, model, data_dict, obj_iter=20,
        ...
        iter_for_sil = sil_iter  # optimize rotation only
        ...
        for it in loop:
            ...
            if it < iter_for_obj:
                ...
                # why do not update trans_init here?

            for i in range(steps_per_iter):
                obj_optimizer.zero_grad() # Nov 1: faster convergence
    ...
    def forward_step(self, model, smpl, data_dict, obj_R, obj_t, obj_s, phase):
        ...
        self.temporal_loss_joint(object, loss_dict, phase)
        self.temporal_loss_objrot(obj_R, loss_dict)
    ...
    def get_obj_center_pred(self, centers_pred_o, data_dict):
        if centers_pred_o.shape[1] == 3:
            assert self.args.model_name in ['tri-agg', 'chore-triplane-vis']
            if self.args.model_name == 'tri-agg':
                obj_center_pred = None
            else:
                # no SMPL center prediction
                obj_center_pred = data_dict['smpl_center'] + torch.mean(centers_pred_o, -1)
        else:
            obj_center_pred = data_dict['smpl_center'] + torch.mean(centers_pred_o[:, 3:, :],
                                                                    -1)  # TODO: why use predicted center??
        return obj_center_pred

    def compute_mask_loss(self, R, obj_s, obj_t, sil, data_dict):
        obj_losses, image, edges, image_ref, edt_ref = sil(R, obj_t, obj_s)
        # loss_dict['mask'] =
        return obj_losses['mask'], edges, edt_ref, image, image_ref
    ...
    def optimize_smpl(self, smpl, data_dict, iter_for_betas=10, iter_for_pose=10,
                      iter_for_kpts=5, steps_per_iter=10,
                      max_iter=100):
                      # max_iter=150):
        """
        ...
        for it in loop:
            ...
            for i in loop_inner:
                smpl_optimizer.zero_grad() # Nov 1, zero grad in each step
    ...
    def forward_smpl(self, smpl, data_dict, phase):
        ...
        self.temporal_loss_smpl(smpl, smpl_verts, loss_dict)
    ...
    def temporal_loss_objrot(self, obj_R, loss_dict):
        "temporal loss for object rotation parameters"
        pass
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pat', default='t*.000', help='pattern to get image files')
