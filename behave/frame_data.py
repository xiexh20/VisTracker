"""
loads more behave for each frame
Author: Xianghui
...
from behave.sync_frame import KinectFrameReader
from behave.seq_utils import SeqInfo


class FrameDataReader(KinectFrameReader):
    "read more behave: pc, mocap, fitted smpl, obj etc"
    def __init__(self, seq, empty=None, ext='jpg', check_image=False):
    ...
    def get_joints3d(self, idx):
        frame_folder = self.get_frame_folder(idx)
        data = np.array(json.load(open(join(frame_folder, 'person/person_J3d.json')))['body_joints3d']).reshape((-1, 4))
        return data

    def get_pare_mesh(self, idx, kid=1):
        pare_file = join(self.get_frame_folder(idx), f'k{kid}.pare.ply')
        return self.load_mesh(pare_file)

    def get_pare_params(self, idx, kid=1):
        pare_file = join(self.get_frame_folder(idx), f'k{kid}.pare.pkl')
        if not isfile(pare_file):
            return None, None
        data = pkl.load(open(pare_file, 'rb'))
        return data['pose'], data['betas']
