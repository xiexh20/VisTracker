...
class KinectCalib:
    ...
    def project_points(self, points, no_distort=False):
        """
        ...
        if no_distort:
            return cv2.projectPoints(points[..., np.newaxis],
                                     ...
                                     self.calibration_matrix, np.zeros_like(self.dist_coeffs))[0].reshape(-1, 2)
        else:
            return cv2.projectPoints(points[..., np.newaxis],
                                     np.zeros(3), np.zeros(3),
                                     self.calibration_matrix, self.dist_coeffs)[0].reshape(-1, 2)
