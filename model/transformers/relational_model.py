...
# adapted from: https://github.com/brjathu/PHALP/tree/master/models/transformers
...
class BoxMultiHeadedAttention(nn.Module):
    ...
    def forward(self, input_query, input_key, input_value, input_box, mask=None):
        ...
        print(f"Relational network: Q: {query.shape}, K: {key.shape}, V: {value.shape}")
...
class RelationTransformerModel(nn.Module):
    ...
    def __init__(self, num_heads, num_layers, num_box_feats):
        """
        default: 1, 1, 6
        Args:
            num_heads: ?
            num_layers:
            num_box_feats: dimension for additional features
        """
    ...
    def forward(self, att_feats, boxes, att_masks=None):
        ...
        """
        print("Transformer input shape:", att_feats.shape, boxes.shape) # demo: [3, 7, 2048]
        x = self.norm1(self.layer1(att_feats, boxes, att_masks))/10. # why divide by 10???
        if self.layers > 1:
            x = x + self.norm2(self.layer2(x, boxes, att_masks))/10.
            if self.layers > 2:
                x = x + self.norm3(self.layer3(x, boxes, att_masks))/10.
        print("Transformer output shape:", x.shape) # demo: [3, 7, 2048]
        return x
