...
class Trainer(object):
    def __init__(self, model, device,
        ...
        self.device = device
        self.lr = lr
        param_groups = self.get_training_params()
        if optimizer == 'Adam':
            self.optimizer = optim.Adam(param_groups, lr=lr)
        if optimizer == 'Adadelta':
            self.optimizer = optim.Adadelta(param_groups)
        if optimizer == 'RMSprop':
            self.optimizer = optim.RMSprop(param_groups, momentum=0.9)
        ...
        print(f"In total {sum(p.numel() for p in model.parameters())} trainable parameters.")
        ...
        self.add_smpl_vect = kwargs.get('add_smpl_vect')
        self.loss_weights = kwargs.get('loss_weights')

    def get_training_params(self):
        "return a list of param groups"
    ...
    def compute_loss(self, batch):
        if self.multi_gpus:
            ...
            query_dict = {}
            if self.add_smpl_vect:
                query_dict['smpl_vect'] = batch.get('smpl_vect').cuda(self.rank, non_blocking=True)  # (B, 3)

            if 'points_posed' in batch:
                query_dict['points_posed'] = batch.get('points_posed').cuda(self.rank, non_blocking=True)  # (B, T, N, 3)

            if 'grid_df' in batch:
                query_dict['grid_df'] = batch.get('grid_df').cuda(self.rank, non_blocking=True)  # (B, reso, reso, reso)
                query_dict['pca_init'] = batch.get('pca_init').cuda(self.rank, non_blocking=True)  # (B, N, 3, 3)

            if 'visibility' in batch:
                query_dict['visibility'] = batch.get("visibility").cuda(self.rank, non_blocking=True) # (B, N)
            ...
            print(f"One forward pass time: {end-start:.2f}") # canonical df model: One forward pass time: 3.34
            return error
    ...
    def train_model(self, epochs):
        ...
        for epoch in tqdm(range(start, epochs)):
            ...
            if self.rank == 0:
                ...
                lr_dict = {f'group{i}':pg['lr'] for i, pg in enumerate(self.optimizer.param_groups)}
                self.writer.add_scalars('lr', lr_dict, epoch)
    ...
    def save_checkpoint(self, epoch, training_time):
        ...
        if not os.path.exists(path):
            if not self.multi_gpus:
                ...
                'scheduler_state_dict':self.lr_scheduler.state_dict()}, path)
            else:
    ...
    def find_best_checkpoint(self, checkpoints, load_last=False):
        """if val min is presented, use that to find, otherwise use the latest one"""
        if not load_last:
            val_min_ck = self.get_val_min_ck()
        else:
            val_min_ck = None # load last checkpoint
        if val_min_ck is None:
            checkpoints = [os.path.splitext(os.path.basename(path))[0].split('_')[-1] for path in checkpoints] # the model trained longest
    ...
    def load_checkpoint(self):
        ...
        path = self.find_best_checkpoint(checkpoints, load_last=True)
        ...
        if 'scheduler_state_dict' in checkpoint:
            ...
            print("Not loading scheduler state")
    ...
    def compute_val_loss(self):
        ...
        sum_val_loss = 0.0
        num_batches = self.get_num_eval_batches(val_loader)
    ...
    def update_vmin_file(self, ck_file, epoch, val_loss):
        ...
        json.dump({'epoch':epoch, 'val_loss':float(val_loss), 'ck_file':ck_file},
                  open(self.exp_path + "best_model.json", 'w'), indent=2)
    ...
    def format_separate_loss(self, separate_losses, length):
        ...
        if len(separate_losses) == 2:
            ...
            # losses for CHORE or CHORE + triplane + visibility
            for name, sp_loss in zip(['df_h', 'df_o', 'parts', 'pca', 'smpl/visibility', 'obj'], separate_losses):
                loss_dict[name] = sp_loss / length
        return loss_dict
